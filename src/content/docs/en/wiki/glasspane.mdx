...
[operator-attention](./operator-attention) for the shipped system. What
...
- [agent-harness](./agent-harness) — the zot/Colibri split that Glasspane observes
- [operator-attention](./operator-attention) — the shipped attention/alert layer over this state machine, including terminal capture + signature triage
- [naming-decisions](./naming-decisions) — `pi_session_id → session_id`, `pi_type → event_type`
