...
The [`wiki-lint`](../../scripts/wiki-lint) script checks every page against the current code: dangling references, resurrected old names (from the naming ledger), and orphan pages. It runs as part of `ci-checks.sh --strict` and is gated by the pre-push hook — a drift failure blocks a push, same as a clippy warning.
