...
- [tui](./tui) — the live terminal dashboard that uses the same `DaemonClient`
- [glasspane](./glasspane) — the pane state machine behind `snapshot`
- [task-board](./task-board) — commands that manipulate the task board
- [store-schema](./store-schema) — SQLite entities queried by the CLI
- [vault-provision](./vault-provision) — why `register-tenant` carries a jail root path
- [external-mcp](./external-mcp) — another daemon client with write-gating
