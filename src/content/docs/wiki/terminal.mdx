...
- [tui](./tui) — the dashboard whose bindings drive this requirement
- [operator-attention](./operator-attention) — the jump keys (`n` / `N`)
  that depend on modifier reporting
- [deployment](./deployment) — the operator USB that ships Kitty + the tmux
  config
