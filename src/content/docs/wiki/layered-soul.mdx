...
([skills-catalog](./skills-catalog)), so the importer intentionally does not target it.
...
1. Implement `system_brain` per the planned schema (see
   [skills-catalog](./skills-catalog)), then extend the importer
...
- [skills-catalog](./skills-catalog) — Colibri's read-only runtime skill consumer
- [store-schema](./store-schema) — planned `system_skills` / `system_brain` tables
