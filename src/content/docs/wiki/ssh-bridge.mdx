...
> **Companion doc:** [mother-hive](./mother-hive) — the _why_ behind each decision
...
- [mother-hive](./mother-hive) — the decisions behind this architecture (why forced-command, why single home, why peer auth, why key-on-seed)
- [hive-routing](./hive-routing) — what the bridge carries: node identity, capabilities, cost-aware task routing
- [external-mcp](./external-mcp) — how `colibri-mcp` hosts external MCP servers (the protocol the bridge plugs into)
- [MOTHER-SETUP.md](../../packaging/mother/MOTHER-SETUP.md) — step-by-step setup instructions, first-run checklist, verification
- [cost-model](./cost-model) — per-task cost tracking (`report-task-cost` over this bridge)
- [cost-dashboard](./cost-dashboard) — mother-side aggregation of costs pushed through this bridge
