...
work, and reports `PASS`/`WARN`/`FAIL`. It's the [task-dispatch-flow](./task-dispatch-flow.md)
...
- [task-dispatch-flow](./task-dispatch-flow.md) — the stall checklist doctor automates
- [factory-model](./factory-model.md) — the harness architecture doctor verifies
- [hive-routing](./hive-routing.md) — per-node identity + capability probes
- [mother-hive](./mother-hive.md) — the MCP-over-SSH path the hive `--json` query rides
