...
> **Companion doc:** [hive-pane](./hive-pane) — the operator presentation
...
| `node-register-mcp`                      | UPSERTs hw_profile into `hive_nodes` on join — reachable over the bridge since `node_register` was allowlisted (#325, see [ssh-bridge](./ssh-bridge)) | No UUID generation at join time                           |
